...
.card{background:rgba(20,20,45,.9);border:1px solid rgba(100,100,180,.2);border-radius:8px;padding:16px}
.card .l{color:#668;font-size:11px;text-transform:uppercase;letter-spacing:1px}
.card .v{font-size:32px;font-weight:700;color:#6f8;margin-top:4px}
.box{background:rgba(20,20,45,.9);border:1px solid rgba(100,100,180,.2);border-radius:8px;padding:16px;margin-bottom:16px}
.box h3{color:#889;font-size:12px;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px}
canvas{width:100%;height:240px}
.cols{display:grid;grid-template-columns:1fr 1fr;gap:16px}
table{width:100%;border-collapse:collapse;font-size:12px}
th{text-align:left;color:#668;font-size:10px;text-transform:uppercase;padding:6px 10px;border-bottom:1px solid rgba(100,100,180,.2)}
td{padding:5px 10px;border-bottom:1px solid rgba(100,100,180,.06);color:#aab}
tr.kept td{color:#6f8} tr.reverted td{color:#f88}
.dot{display:inline-block;width:7px;height:7px;border-radius:50%;margin-right:5px}
.dot.kept{background:#6f8}.dot.reverted{background:#f66}
.btn{padding:8px 18px;font-size:13px;cursor:pointer;border:none;border-radius:6px;font-weight:600}
.tab{padding:7px 16px;font-size:12px;cursor:pointer;border:1px solid rgba(100,100,180,.2);border-bottom:none;border-radius:6px 6px 0 0}
...
<div class="row">
<button class="btn" id="go" onclick="api('start')" style="background:#2a4;color:#fff">Start</button>
<button class="btn" id="no" onclick="api('stop')" style="background:#c44;color:#fff" disabled>Stop</button>
<span id="st" style="color:#667;font-size:13px;margin-left:8px">Idle</span></div>
<div class="row" style="gap:2px">
<button class="tab on" id="t-perf" onclick="T('perf')">Performance (Browser)</button>
<button class="tab off" id="t-compute" onclick="T('compute')">Compute (Deno)</button>
<button class="tab off" id="t-quality" onclick="T('quality')">Quality</button></div>
<div class="grid4">
<div class="card"><div class="l" id="lb">Max Boids</div><div class="v" id="vb">-</div></div>
<div class="card"><div class="l">Baseline</div><div class="v" id="vbl">118,000</div></div>
...
<div class="cols">
<div class="box"><h3 id="c1t">Max Boids Over Time</h3><canvas id="c1"></canvas></div>
<div class="box"><h3>All Experiments</h3><canvas id="c2"></canvas></div></div>
<div class="cols">
<div class="box"><h3>Experiment Log</h3><div style="max-height:240px;overflow-y:auto">
<table><thead><tr><th>#</th><th id="cv">Value</th><th>Description</th><th>Result</th></tr></thead>
<tbody id="tb"></tbody></table></div></div>
<div class="box"><h3>Git Log</h3><div id="gl" style="max-height:240px;overflow-y:auto;font:11px/1.5 monospace;color:#99a"></div></div></div>
<div class="box"><h3>Agent Log</h3><pre id="al" style="max-height:160px;overflow-y:auto;font:10px/1.4 monospace;color:#889;white-space:pre-wrap;margin:0"></pre></div>
<div id="ts"></div>
<script>
const CFG={perf:{lbl:'Max Boids',bl:'118,000',dk:'perf_experiments',bk:'perf_best',sc:0},
compute:{lbl:'Max Boids (Deno)',bl:'170,000',dk:'compute_experiments',bk:'compute_best',sc:0},
quality:{lbl:'Best Score',bl:'0.695',dk:'quality_experiments',bk:'quality_best',sc:1}};
let tab='perf';
...
const c=CFG[tab],ex=d[c.dk]||[],best=d[c.bk]||0,kept=ex.filter(e=>e.result==='kept');
const f=v=>c.sc?v.toFixed(4):v.toLocaleString();
const yF=c.sc?(v=>v.toFixed(2)):(v=>v<1e4?v.toFixed(0):(v/1e3).toFixed(0)+'k');
