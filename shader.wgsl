...
fn flock_radius_linked(@builtin(global_invocation_id) id: vec3u) {
  ...
  var new_vel = boid.vel;
  let nf = max(f32(n_align), 1.0);
  // Combined alignment + cohesion with shared scaling
  let target_vel = ali / nf;
  let target_pos = coh / nf;
  new_vel += (target_vel - boid.vel) * params.align_factor * 10.0;
  new_vel += (target_pos - boid.pos) * params.cohesion_factor;
  new_vel += sep * params.separation_factor * 0.6;
