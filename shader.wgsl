...
fn flock_radius_linked(@builtin(global_invocation_id) id: vec3u) {
  ...
  // Slowly rotating horizontal wind with time-varying strength
  let wind_angle = f32(params.frame_count) * 0.005;
  let wind_strength = 1.5 + sin(f32(params.frame_count) * 0.002) * 0.5;
  new_vel.x += sin(wind_angle) * wind_strength;
  new_vel.z += cos(wind_angle) * wind_strength;
