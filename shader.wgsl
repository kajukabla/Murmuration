...
fn flock_radius_linked(@builtin(global_invocation_id) id: vec3u) {
  ...
  // Phase-shifting wind: position-dependent phase creates wave patterns
  let base_angle = f32(params.frame_count) * 0.005;
  let phase_offset = dot(boid.pos.xz, vec2f(0.02, 0.02));
  let wind_a = base_angle + phase_offset;
  new_vel.x += sin(wind_a) * 2.0;
  new_vel.z += cos(wind_a) * 2.0;
