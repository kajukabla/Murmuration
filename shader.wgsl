...
fn flock_radius_linked(@builtin(global_invocation_id) id: vec3u) {
  ...
  // Speed clamp (no turn rate limiter — sharper responses)
  let spd_sq_linked = dot(new_vel, new_vel);
  let max_spd_linked = params.max_speed;
  if (spd_sq_linked > max_spd_linked * max_spd_linked) {
    new_vel *= max_spd_linked * inverseSqrt(spd_sq_linked);
  }
  let min_spd_linked = params.min_speed;
  if (spd_sq_linked < min_spd_linked * min_spd_linked && spd_sq_linked > 0.0001) {
    new_vel *= min_spd_linked * inverseSqrt(spd_sq_linked);
  }
  ...
  let linked_vel_dir = new_vel * inverseSqrt(max(dot(new_vel, new_vel), 0.0001));
  boids_dst[i].heading = linked_vel_dir;
