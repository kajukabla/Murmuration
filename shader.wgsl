...
fn flock_radius(@builtin(global_invocation_id) id: vec3u) {
  ...
  // Update heading for rendering (reuse spd_sq, skip old_h renorm — already unit length)
  let inv_spd = inverseSqrt(max(spd_sq, 0.0001));
  let vel_dir = new_vel * inv_spd;
  boids_dst[i].heading = normalize(mix(boid.heading, vel_dir, 0.12));

  // Viz metrics (reuse cached spd_sq)
  boids_dst[i].speed = spd_sq * inv_spd;  // sqrt(x) == x * inverseSqrt(x)
