...
fn flock_radius_linked(@builtin(global_invocation_id) id: vec3u) {
  ...
  // Read exactly 1 neighbor from linked list head (simplest possible)
  var new_vel = boid.vel;
  let j = atomicLoad(&cell_counts[my_ci]);
  if (j != 0xFFFFFFFFu) {
    new_vel += (boids_src[j].vel - boid.vel) * params.align_factor * 12.0;
    new_vel += (boids_src[j].pos - boid.pos) * params.cohesion_factor;
  }
