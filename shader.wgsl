...
fn drift(@builtin(global_invocation_id) id: vec3u) {
  ...
  // Ellipsoidal boundary — cheap spherical pre-test skips ~80% of boids
  let pos_d2 = dot(boid.pos, boid.pos);
  let r = params.sphere_radius;
  let inner_r = (r - r * 0.15) * 0.7; // conservative inner sphere (accounts for Y scaling)
  if (pos_d2 > inner_r * inner_r) {
