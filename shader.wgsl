...
fn flock(@builtin(global_invocation_id) id: vec3u) {
  ...
  // Speed-dependent perturbation: slower boids more likely to be perturbed
  let speed_ratio = length(boid.vel) / params.max_speed;
  let perturb_threshold = mix(0.12, 0.04, speed_ratio); // 4-12% depending on speed
  let perturb_hash = fract(sin(f32(i * 7919u + params.frame_count * 104729u)) * 43758.5);
  if (perturb_hash < perturb_threshold) {
