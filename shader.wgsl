...
fn flock_radius_linked(@builtin(global_invocation_id) id: vec3u) {
  ...
  // Minimal: read boid + cell head, apply alignment + gravity + wind
  let boid = boids_src[i];
  var new_vel = boid.vel;

  // Quick alignment: just use cell head neighbor
  let ci = cell_index(get_cell(boid.pos));
  let head = atomicLoad(&cell_counts[ci]);
  if (head != 0xFFFFFFFFu && head != i) {
    let other_vel = boids_src[head].vel;
    new_vel += (other_vel - boid.vel) * (params.align_factor * 12.0);
    new_vel += (boids_src[head].pos - boid.pos) * (params.cohesion_factor * 16.0);
  }
