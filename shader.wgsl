...
fn flock_radius_linked(@builtin(global_invocation_id) id: vec3u) {
  ...
  // Sequential neighbor access (skip grid lookup — use adjacent indices)
  let nb = params.num_boids;
  let j0 = select(i - 1u, nb - 1u, i == 0u);
  let j1 = select(i + 1u, 0u, i >= nb - 1u);
  let j2 = select(i + 2u, 1u, i >= nb - 2u);
